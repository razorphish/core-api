        ...
        async.waterfall([
            ...
            (requestToken, accessToken, done) => {
                ...
                tokenRepo.insert(accessToken, (error, result) => {
                    if (error) {
                        logger.error(`${this._classInfo}.callBack():4:insert [${this._routeName}]`, error);
                        response.status(500).send(error);
                    } else {
                        if (result) {
            ...
            (accessToken, done) => {
                ...
                twitterLibrary.verifyCredentials(
                    ...
                    (body, limits) => {
                        if (!body) {
                            let error = 'Unable to verify users credentials';
                            logger.error(`${this._classInfo}.callBack():3:verifyCredentials [${this._routeName}]`, error);
                            response.status(500).send(error);
                        } else {
                            logger.debug(`${this._classInfo}.callBack():3:verifyCredentials [${this._routeName}] OK`);
            ...
            (accessToken, userCredentials, done) => {
                ...
                twitterUserRepo.insert(twitterUser, (error, result) => {
                    if (error) {
                        logger.error(`${this._classInfo}.callBack():4:insertTwitterUser [${this._routeName}]`, error);
                        response.status(500).send(error);
                    } else {
                        if (result) {
        ...
        async.waterfall([
            ...
            (twitterRequestToken, done) => {
                ...
                tokenRepo.byToken(accessToken, (error, accessToken) => {
                    if (error) {
                        logger.error(`${this._classInfo}.request_token():2:byToken [${this._routeName}]`, error);
                        response.status(500).send(error);
                    } else {
                        if (accessToken) {
                            logger.debug(`${this._classInfo}.request_token():2:byToken [${this._routeName}] OK`);
            ...
            (accessToken, twitterRequestToken, done) => {
                ...
                tokenRepo.insert(requestToken, (error, result) => {
                    if (error) {
                        ...
                        if (result) {
                            logger.debug(`${this._classInfo}.request_token()::insert [${this._routeName}] OK`);
                            //Do not return token
