...
class ApplicationSettingsController {
  ...
  insertEmailNotification(request, response) {
    ...
    logger.info(
      `${this._classInfo}.insertEmailNotification(${id}) [${this._routeName}]`
    );

    repo.insertEmailNotification(id, request.body, (error, result) => {
      if (error) {
        logger.error(
          `${this._classInfo}.insertEmailNotification() [${this._routeName}]`,
          error,
          request.body
        );
        response.status(500).json(error);
        ...
        logger.debug(
          `${this._classInfo}.insertEmailNotification() [${this._routeName}] OK`
        );
        response.json(result);
      }
    });
  ...
  insertNotification(request, response) {
    ...
    logger.info(
      `${this._classInfo}.insertNotification(${id}) [${this._routeName}]`
    );
  ...
  insertNotificationAction(request, response) {
    const { id } = request.params; // SettingsId
    const { notificationId } = request.params; // Notification id

    logger.info(
      `${this._classInfo}.insertNotificationAction(${id},${notificationId}) [${this._routeName}]`
    );

    repo.insertNotificationAction(
      id,
      notificationId,
      request.body,
      (error, result) => {
        if (error) {
          logger.error(
            `${this._classInfo}.insertNotificationAction() [${this._routeName}]`,
            error,
            request.body
          );
          response.status(500).json(error);
          ...
          logger.debug(
            `${this._classInfo}.insertNinsertNotificationActionotification() [${this._routeName}] OK`
          );
          response.json(result);
        }
      }
