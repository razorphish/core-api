...
class UserRepository {
  ...
  addDevice(userId, device, callback) {
    ...
    UserModel.findByIdAndUpdate(
      ...
      .catch((error) => {
        logger.error(
          `${this._classInfo}.addDevice(${userId})::findByIdAndUpdate`,
          error
        );
  ...
  addNotification(userId, body, callback) {
    ...
    UserModel.findByIdAndUpdate(
      ...
      .catch((error) => {
        logger.error(
          `${this._classInfo}.addNotification(${userId})::findByIdAndUpdate`,
          error
        );
        return callback(error);
      });
  ...
  byRefreshToken(token, callback) {
    ...
    UserModel.findOne({ 'refreshToken.value': token }, null, {
      select: {
        password: 0,
        salt: 0,
        loginAttempts: 0,
  ...
  byRole(role, callback) {
    ...
    UserModel.find({ roles: { $elemMatch: { name: role } } }, null, {
      select: {
        password: 0,
        salt: 0,
        refreshToken: 0,
  ...
  byUsername(username, callback) {
    ...
    UserModel.findOne({ username }, null, {
      select: {
        password: 0,
        salt: 0,
        refreshToken: 0,
    ...
    UserModel.findOne({ username })
      .then((data) => {
        data.comparePassword(password, (error, isMatch) => {
          if (error) {
            logger.error(
              `${this._classInfo}.passwordMatch(${username}, ${password})::comparePassword`,
              error
            );
            return callback(error);
          }
  ...
  updateProfile(id, body, callback) {
    ...
    UserModel.findOneAndUpdate({ _id: id }, body, {
      new: true,
      select: {
        addresses: 0,
        devices: 0,
      ...
      .catch((error) => {
        logger.error(
          `${this._classInfo}.updateProfile(${id})::findOneAndUpdate`,
          error
        );
        return callback(error);
      });
