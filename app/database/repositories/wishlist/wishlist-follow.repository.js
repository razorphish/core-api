...
class WishlistFollowRepository {
  ...
  get(id, callback) {
    ...
    WishlistFollowModel.findById(id, null, {
      select: {
        accountId: 1
      }
    })
  ...
  unfollow(wishlistFollowId, body, callback) {
    ...
    WishlistFollowModel.findOneAndUpdate({ _id: wishlistFollowId }, body, {
      new: true
    })
      .then((data) => {
      ...
      .catch((error) => {
        logger.error(
          `${this._classInfo}.unfollow(${wishlistFollowId})::findOneAndUpdate`,
          error
        );
        return callback(error);
      });
  ...
  update(id, body, callback) {
    ...
    WishlistFollowModel.findOneAndUpdate({ _id: id }, body, { new: true })
      .then((data) => {
        // returns WishlistFollow data
        callback(null, data);
      })
