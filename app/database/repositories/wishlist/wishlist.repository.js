...
class WishlistRepository {
  ...
  byUserId(userId, callback) {
    ...
    WishlistModel.find({
      ...
      .populate({
        ...
        select:
          '_id userId name categoryId price quantity url notes purchased purchasedBy image statusId sortOrder dateCreated',
        populate: [
          {
            path: 'userId',
      ...
      .populate({
        path: 'follows',
        select:
          '_id userId endpoint expirationTime keys notifiedOnAddItem notifiedOnRemoveItem notifiedOnCompletion statusId',
        match: { statusId: { $ne: 'deleted' } },
        populate: [
          {
            path: 'userId',
  ...
  get(id, callback) {
    ...
    WishlistModel.findById(id, null, {
      select: {
        _id: 1,
        statusId: 1,
        privacy: 1,
      ...
      .populate({
        ...
        match: { statusId: { $ne: 'deleted' } },
        select:
          '_id name categoryId price quantity url notes purchased purchasedBy image statusId sortOrder dateCreated'
      })
      .populate({
        path: 'follows',
        select:
          '_id userId endpoint expirationTime keys notifiedOnAddItem notifiedOnRemoveItem notifiedOnCompletion',
        match: { statusId: { $ne: 'deleted' } }
      })
      .then((data) => {
        callback(null, data);
  ...
  getDetails(wishlistId, callback) {
    ...
    WishlistModel.findById(wishlistId, null, {})
      .populate({
        path: 'userId',
        select: '_id firstName lastName email username'
      })
  ...
  search(query, fieldSelect, callback) {
    ...
    WishlistModel.find(query, fieldSelect)
      .then((data) => {
        callback(null, data);
      })
      .catch((error) => {
  ...
  update(id, body, callback) {
    ...
    WishlistModel.findOneAndUpdate({ _id: id }, body, { new: true })
      .then((data) => {
